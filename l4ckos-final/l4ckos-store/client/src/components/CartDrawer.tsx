...
export default function CartDrawer() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => { if (event.key === "Escape") closeCartDrawer(); };
    document.addEventListener("keydown", onKeyDown);
    return () => { document.body.style.overflow = previousOverflow; document.removeEventListener("keydown", onKeyDown); };
  }, [closeCartDrawer, isCartDrawerOpen]);

  const goTo = (path: "/produtos" | "/carrinho" | "/checkout") => { closeCartDrawer(); navigate(path); };

  return <div className={`l4-cart-drawer-layer ${isCartDrawerOpen ? "is-open" : ""}`} aria-hidden={!isCartDrawerOpen}>
