...
const orderSummaryStatus = (order: { payment?: { status?: string | null } | null; fulfillmentStatus?: string | null }) => {
  if (order.fulfillmentStatus === "delivered") return "Entregue";
  if (order.fulfillmentStatus === "shipped") return "Enviado";
  if (order.fulfillmentStatus === "processing") return "Em separação";
  if (["confirmed", "received", "partially_refunded"].includes(String(order.payment?.status))) return "Pagamento confirmado";
